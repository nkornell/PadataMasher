...
// --- the stuff below is probably lower priority ---
...
// -------- it could flag each problematic row individually (e.g., notice that the same deviation always happens on trial 11). 
// ------------ Maybe put "expected deviation" in the additional info column for that row. 
// - how to do it
// --- when comparing files, and when looking at perfect files, 
// --- make an array of all rows with error messages, with index of the trial number (not row)
// --- then, for each trial number, compute the modeal error message. 
// --- if it happens in just about every file (i.e., frequency >= numfiles -1) include it in an array called expectedDeviation.
// --- for example, expectedDeviation(19) = "Wrong number of ..." 
// --- then go through all of the rows that have errors. If the error for the trial number equals the expected error, 
// --- add to the start of the row notes string "Expected deviation." 

// ---- other ways of doing user interface (probably not as good as putting info in the additional column, though)
// -------- make a separate function for finding signature column values and use it on stepouts maybe? or maybe on ALL rows, grouping them by trial number???
// -------- re-run columnType on them? Although this might lump them all into one category. 
// -------- it could then also re-run columnType on good rows. 
// ---- it could categorize them and then 
// -------- tell you how many there are of each type 
// -------- ask whether you want to exclude each category. 
