...
function buildVariableSelectorTable_lenientScorer() {
	var i = 0;
	console.log( "buildVariableSelectorTable_lenientScorer")
	console.log( inputCell[0].length)
...
// 		out = out + "<tr><td>" + tempColName + " </td>";
// 		out = out + "<td><select id='"+i+"' name='"+tempColName+"'>";
// 		out = out + "<option value='Not Using'>---</option>";
		out = out + "<td style='vertical-align: top'><b>" + tempColName + "</b><br>";
		for (j = 0; j < column_data[tempColName].level_text.length; j++) {
tempItemValue = column_data[tempColName].sorted_array[j][0];
tempID = tempColName+'**'+tempItemValue;
out = out + '<input type="checkbox" id="'+tempID+'"><label for="'+tempID+'">'+tempItemValue+'</label><br>'

// 			out = out + "<option value='"+column_data[tempColName].sorted_array[j][0]+"'>"+column_data[tempColName].sorted_array[j][0]+" ["+column_data[tempColName].sorted_array[j][1]+"]</option>";
			if (j > 9) {break;} // this means it never gives you more than 10 options
		}
		out = out + "</td>";
	}
...
// 	for (i = 0; i < inputCell.length; i++) {
// 		tempLevel = inputCell[i][betweenColumnsSelected]
// 
// 		if (tempLevelArray.indexOf(tempLevel) == -1) { // it's identified a novel unique between-level combination
// 			tempLevelArray.push(tempLevel);
// 		}
// 	}
// // 	if (betweenColumnsSelected.length == 1 && remember == true) {
// 	// if it's during setup, set the tooltips
// 	var columnName = tempLevelArray.shift(); // get rid of the first element, which is the column name
// 	
// 	numLevels = tempLevelArray.length;
// 
// 
// 	var ttcString = ""//"<u>Unique values (" + numLevels[betweenColumnsSelected] + ")</u><br>";
// 	for (i = 0; i < tempLevelArray.length; i++) {
// 		ttcString += tempLevelArray[i] + "<br>";
// 		// show a maximum of 8 rows in the tooltips
// 		if (i == 7 && tempLevelArray.length > 9) {
// 			ttcString += "<i>(etc...)</i>";
// 			break;
// 		}
// 	}		
...
function column_data_object(input_name, input_values) { 
	...
	this.compute_levels = function(input_search_status) {
		...
		//dfd  do a thing where if there's more than x levels, only add x of them? 
...
// 		console.log( temp_array);
		...
		for (i = 0; i < this.level_text.length; i++) {
//			console.log(this.level_num_observations[this.level_text[i]] + ' ' + this.level_text[i])
		}
	}
}
...
function buildTableLenientScorer(typ) {
	var out = "";
	if (typ == "input") {
